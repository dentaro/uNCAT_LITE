...
static ringbuf_handle_t rb_create(uint32_t size)
{
    ...
    do {
        bool _success =
            (
                (rb             = (ringbuf_handle_t)heap_caps_malloc(sizeof(ringBuf), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL)) &&
                (buf            = (char*)heap_caps_malloc(size, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL))   &&
...
static void IRAM_ATTR timer_group_isr(void *para)
{
    ...
    handle->timg_dev->hw_timer[handle->config.timer_num].config.alarm_en = TIMER_ALARM_EN;#endif /**< CONFIG_IDF_TARGET_ESP32S3 */
...
esp_err_t pwm_audio_init(const pwm_audio_config_t *cfg)
{
    ...
    handle = (pwm_audio_handle_t)heap_caps_malloc(sizeof(pwm_audio_handle), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    ...
    timer_config_t config = {(timer_alarm_t)0}; ///
    ...
    res = pwm_audio_set_param(16000, (ledc_timer_bit_t)8, 2);
...
esp_err_t pwm_audio_deinit(void)
{
    ...
    for (size_t i = 0; i < PWM_AUDIO_CH_MAX; i++) {
        if (handle->ledc_channel[i].gpio_num >= 0) {
            gpio_set_direction((gpio_num_t)handle->ledc_channel[i].gpio_num, GPIO_MODE_INPUT);
        }
    }
